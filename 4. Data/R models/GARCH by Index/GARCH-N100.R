...
plot(abs(N1001_return_t$N1001_return_t)~N1001[2:nrow(N1001),1],type='l',lwd=1
     ,main='Euronext 100 GARCH (1,1) Training Dataset',
     xlab='Date',ylab='Volatility'
     ,col='darkblue')
lines(abs(as.numeric(N1001[2:nrow(N1001),5]))
      ~N1001[2:nrow(N1001),1],type = 'l',col='darkorange',lwd=2) # We can see that Model not cover a lot
legend('topleft',
...
plot(as.numeric(N1001[2:nrow(N1001),6])~N1001[2:nrow(N1001),1], type='l',xlab='Date'
     ,ylab='Volatility',main='Euronext 100 Inneficiency of GARCH (1,1) Model on Training Dataset',
     col='darkblue')
...
plot(abs(as.numeric(N1002[2:nrow(N1002),3]))~N1002[2:nrow(N1002),1],type='l',lwd=1
     ,main='Euronext 100 GARCH (1,1) Test Set',
     xlab='Date',ylab='Volatility'
     ,col='darkblue')
lines(abs(as.numeric(N1002[2:nrow(N1002),5]))
      ~N1002[2:nrow(N1002),1],type = 'l',col='darkorange',lwd=0.1) 
legend('topleft',
       legend = c('Absolute Returns','Conditional SD'),
       col = c('darkblue','darkorange'),
       pch = c(9,9))
