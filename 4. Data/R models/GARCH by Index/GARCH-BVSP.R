...
#BVSP GARCH Model
BVSP1_s<- ugarchspec(variance.model = list(model = "sGARCH", garchOrder = c(1,1)),
                     mean.model = list(armaOrder = c(1,1), include.mean = TRUE),
                     distribution.model = "std")
ctrl = list(RHO = 1,DELTA = 1e-8,MAJIT = 100,MINIT = 650,TOL = 1e-6)
BVSP1_m<-ugarchfit(data = BVSP1_return_t,spec =BVSP1_s,solver = 'solnp',solver.control = ctrl)
BVSP1_m
...
plot(abs(BVSP1_return_t$BVSP1_return_t)~BVSP1[2:nrow(BVSP1),1],type='l',lwd=1
     ,main='IBOVESPA GARCH (1,1) Training Dataset',
     xlab='Date',ylab='Volatility'
     ,col='darkblue')
lines(abs(as.numeric(BVSP1[2:nrow(BVSP1),5]))
      ~BVSP1[2:nrow(BVSP1),1],type = 'l',col='darkorange',lwd=2) # We can see that Model not cover a lot
legend('topleft',
...
plot(as.numeric(BVSP1[2:nrow(BVSP1),6])~BVSP1[2:nrow(BVSP1),1], type='l',xlab='Date'
     ,ylab='Volatility',main='IBOVESPA Inneficiency of GARCH (1,1) Model on Training Dataset',
     col='darkblue')
...
plot(abs(as.numeric(BVSP2[2:nrow(BVSP2),3]))~BVSP2[2:nrow(BVSP2),1],type='l',lwd=1
     ,main='IBOVESPA GARCH (1,1) Test Set',
     xlab='Date',ylab='Volatility'
     ,col='darkblue')
lines(abs(as.numeric(BVSP2[2:nrow(BVSP2),5]))
      ~BVSP2[2:nrow(BVSP2),1],type = 'l',col='darkorange',lwd=0.1) 
legend('topleft',
       legend = c('Absolute Returns','Conditional SD'),
       col = c('darkblue','darkorange'),
       pch = c(9,9))
