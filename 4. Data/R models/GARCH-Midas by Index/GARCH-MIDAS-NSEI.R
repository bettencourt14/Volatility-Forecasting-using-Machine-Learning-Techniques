...
par(mar=c(5,5,5,5))
plot(NSEI_MIDAS_Data$Date,y= NSEI_MIDAS_Data$Value_LTIR,type='l',
     col='darkblue',xlab='Date',ylab='Long Term Interest Rate % Value')
par(new=T)
plot(NSEI_MIDAS_Data$Date,y=NSEI_MIDAS_Data$Value_House,type='l',
     col='darkorange',ylab='',xlab='',axes=F,
     main=' India Long Term Interest Rate and House Pricing')
axis(side=4,at=pretty(range(NSEI_MIDAS_Data$Value_House,na.rm=T)))
...
plot(abs(as.numeric(NSEI1_return[NSEI1_lower:NSEI1_total,1]))*100
     ...
     main='NIFTY 50 GARCH-MIDAS Training Dataset',
     col='darkblue')
...
plot(abs(NSEI2_returns[,])~NSEI2$Date,type='l',col='darkblue',
     xlab='Date',ylab='Volatility', main='NIFTY 50 GARCH-MIDAS Test',
     ylim=c(0,0.07))
lines(NSEI_GARCH_MIDAS_forecast_SD$e~NSEI2$Date,type='l',col='darkorange')
legend("topleft",
