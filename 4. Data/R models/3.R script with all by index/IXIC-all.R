...
IXIC2_view<-cbind(IXIC2_actual_returns,IXIC_error_means)
IXIC2_view
...
for (i in 1:nrow(as.data.frame(IXIC_error_means))) {
  ...
  e<- abs(IXIC2_view[i,1]-IXIC2_view[i,2])
  e<-as.data.frame(e)
...
for (i in 1:nrow(as.data.frame(IXIC_error_means))) {
  ...
  g<- abs(IXIC2_view[i,1]-IXIC2_view[i,2])**2
  g<-as.data.frame(g)
...
IXIC_mean_error
# Mean Errors
IXIC_absoulte_error<- sum(IXIC_abs_error$e)/nrow(IXIC_abs_error)
IXIC_squared_mean_error<- sum(IXIC_mean_error$g)/nrow(IXIC_mean_error)
IXIC_root_mean_error<-sqrt(IXIC_squared_mean_error)
