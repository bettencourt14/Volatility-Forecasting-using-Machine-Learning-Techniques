...
source("E:/Thesis Francisco Bettencourt/Tese Nov/4. Data/R models/High frequency preliminary analysis/NSEI-high.R")


# Set the training set and test set size
train_size <- nrow(NSEI1)-nrow(NSEI2)
test_size <- nrow(NSEI2)*2
...
return<-diff(log(NSEI$Close))
return<-rbind(NA,as.data.frame(return))
return[is.na(return)]<-0
full_data<-cbind(as.data.frame(NSEI$Date)
...
history <- model %>% fit(x = train_x,
                         ...
                         epochs = epochs,
                         verbose = 1)
plot(history,main='NIFTY 50 LSTM Loss Function and MAE')
...
plot(abs(res$train)~res$dates
     ...
     main='NIFTY 50 LSTM Training Capacity',lwd=1)
lines(abs(res$pred_train)~res$dates,type='l',col='orange',lwd=01)

# Error of Training set

NSEI_LSTM_abs_value<-res$train

for (i in nrow(NSEI2)+1:nrow(res)) {
  p<-abs(res[i,2]-res[i,4])
  p<-as.data.frame(p)
  
  if(i==nrow(NSEI2)+1){
    
    NSEI_LSTM_abs_value<-p
  }
  else{
    NSEI_LSTM_abs_value<-rbind(NSEI_LSTM_abs_value,p)
  }  
}
d<-length(which(is.na(NSEI_LSTM_abs_value)))
NSEI_LSTM_training_mean_abs_error<-sum(NSEI_LSTM_abs_value$train,na.rm=T)/(nrow(NSEI_LSTM_abs_value)-d)


NSEI_LSTM_squared_value<-NSEI_LSTM_abs_value
...
for (i in nrow(NSEI2)+1:nrow(res)) {
  ...
  if(i==nrow(NSEI2)+1){
    ...
    NSEI_LSTM_squared_value<-q
  }
  else{
    NSEI_LSTM_squared_value<-rbind(NSEI_LSTM_squared_value,q)
  }  
}
e<-length(which(is.na(NSEI_LSTM_squared_value)))

NSEI_LSTM_training_mean_abs_error
NSEI_LSTM_training_squared_error<-sum(NSEI_LSTM_squared_value,na.rm=T)/
  (nrow(NSEI_LSTM_abs_value)-e)
NSEI_LSTM_training_root_error<-sqrt(NSEI_LSTM_training_squared_error)


# plot of Test
test_NSEI<-as.data.frame(res[length(which(is.na(res$pred_test))):nrow(res),5])
dim(test_NSEI)
head(test_NSEI,8)
test_NSEI<-as.data.frame(test_NSEI[2:nrow(test_NSEI),1])

plot(abs(NSEI2_returns$`diff(log(NSEI2$Close))`)~NSEI2$Date
     ,type='l',main=' NIFTY 50 LSTM forecasting capaacity',
     xlab='Date',ylab='Absolute Return')
lines(abs(test_NSEI[,1])~NSEI2$Date,col='orange',type='l')

# test errors
  
for (i in 1:nrow(NSEI2)) {
  
  u<- abs(test_NSEI[i,1]-NSEI2_returns[i,1])
  u<-as.data.frame(u)
  
  if (i==1) {
    NSEI_LSTM_abs_error_value<- u
  }
  else{
    NSEI_LSTM_abs_error_value<-rbind(NSEI_LSTM_abs_error_value,u)
  }
}

NSEI_LSTM_abs_error_value

for (i in 1:nrow(NSEI2)) {
  
  w<- abs(test_NSEI[i,1]-NSEI2_returns[i,1])**2
  w<-as.data.frame(w)
  
  if (i==1) {
    NSEI_LSTM_squared_error_value<- w
  }
  else{
    NSEI_LSTM_squared_error_value<-rbind(NSEI_LSTM_squared_error_value,w)
  }
}
NSEI_LSTM_squared_error_value


NSEI_LSTM_abs_error<-sum(NSEI_LSTM_abs_error_value$u)/nrow(NSEI_LSTM_abs_error_value)
NSEI_LSTM_squared_error<- sum(NSEI_LSTM_squared_error_value$w)/nrow(NSEI_LSTM_squared_error_value)
NSEI_LSTM_root_error<-sqrt(NSEI_LSTM_squared_error)
