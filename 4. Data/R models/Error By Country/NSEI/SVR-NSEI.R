...
plot(NSEI_tune, main='NIFTY 50 SVR Model Performance on Training Dataset',col='darkblue')
...
plot(abs(as.numeric(NSEI_Brent_Corn$`Return NSEI`))~NSEI_Brent_Corn$Date,type='l',col='darkblue',
     main='NIFTY 50 SVR Training', xlab='Date',ylab='Volatility')
lines(abs(NSEI_SVR_predicted_best_model)~
        NSEI_Brent_Corn$Date,col='darkorange')
legend('topleft',
...
plot(NSEI_forecast_tune, main=' NIFTY 50 SVR Model Performance Test Dataset')
...
plot(abs(as.numeric(NSEI2_returns$`diff(log(NSEI2$Close))`))~NSEI2$Date,type='l',col='darkblue',
     main='NIFTY 50 SVR Test', xlab='Date',
     ylab='Return',ylim=c(0,0.1))
lines(abs(NSEI_SVR_forecast)~
        NSEI2$Date,col='darkorange')
legend('topleft',
       legend=c('Absolute Actual Return','SVR Forecast'),
       col=c('darkblue','darkorange'),
       pch = c(9,9))
...
# NSEI Training Error function for Optimal model
NSEI_SVR_forecast<-as.data.frame(NSEI_SVR_forecast)
NSEI_actual_returns_test<-as.data.frame(NSEI_actual_returns_test)

for (i in 1:nrow(NSEI_actual_returns_test)) {
  d<-abs(NSEI_SVR_forecast[i,1]-NSEI_actual_returns_test[i,1])
  d<-as.data.frame(d)
...
for (i in 1:nrow(NSEI_actual_returns_test)) {
  d<-abs(NSEI_SVR_forecast[i,1]-NSEI_actual_returns_test[i,1])**2
  d<-as.data.frame(d)
