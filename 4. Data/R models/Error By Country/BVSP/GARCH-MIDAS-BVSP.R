...
par(mar=c(5,5,5,5))
plot(BVSP_MIDAS_Data$Date,y= BVSP_MIDAS_Data$Value_LTIR,type='l',
     col='darkblue',xlab='Date',ylab='Long Term Interest Rate % Value')
par(new=T)
plot(BVSP_MIDAS_Data$Date,y=BVSP_MIDAS_Data$Value_House,type='l',
     col='darkorange',ylab='',xlab='',axes=F,
     main=' Brasil Long Term Interest Rate and House Pricing')
axis(side=4,at=pretty(range(BVSP_MIDAS_Data$Value_House,na.rm=T)))
...
plot(abs(as.numeric(BVSP1_return[BVSP1_lower:BVSP1_total,1]))*100
     ...
     main='IBOVESPA GARCH-MIDAS Training Dataset',
     col='darkblue')
...
plot(abs(BVSP2_returns[,])~BVSP2$Date,type='l',col='darkblue',
     xlab='Date',ylab='Volatility', main='IBOVESPA GARCH-MIDAS Test',
     ylim=c(0,0.07))
lines(BVSP_GARCH_MIDAS_forecast_SD$e~BVSP2$Date,type='l',col='darkorange')
legend("topleft",
