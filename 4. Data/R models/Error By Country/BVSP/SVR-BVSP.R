...
plot(BVSP_tune, main='IBOVESPA SVR Model Performance on Training Dataset',col='darkblue')
...
plot(BVSP_SVR_Data$`BVSP Return`~BVSP_SVR_Data$`Lag Return`,xlab='Lag Returns',
     ylab='IBOVESPA  Returns', main='IBOVESPA  vs. Lag Returns Multiple Model Capacity',
     type='p',lwd=3)
points(BVSP_SVR_Data$`BVSP Return`,BVSP_lr_predicted,col='blue',pch=4)
points(BVSP_SVR_Data$`BVSP Return`,BVSP_SVR_predicted,col='red',pch=4)
points(BVSP_SVR_Data$`BVSP Return`,BVSP_SVR_predicted_best_model,col='orange',pch=4)
...
plot(abs(as.numeric(BVSP_Brent_Corn$`Return BVSP`))~BVSP_Brent_Corn$Date,type='l',col='darkblue',
     main='IBOVESPA SVR Training', xlab='Date',ylab='Volatility')
lines(abs(BVSP_SVR_predicted_best_model)~
        BVSP_Brent_Corn$Date,col='darkorange')
legend('topleft',
...
set.seed(994)
BVSP_forecast_tune<-tune(svm,as.numeric(BVSP_SVR_total_Data$`BVSP Return`)~
                          as.numeric(BVSP_SVR_total_Data$`Lag Return`)+
                          as.numeric(BVSP_SVR_total_Data$`Brent Return`) +
                          as.numeric(BVSP_SVR_total_Data$`Corn Return`)+
                          as.numeric(BVSP_SVR_total_Data$`Lag Vol`),
...
plot(BVSP_forecast_tune, main=' IBOVESPA SVR Model Performance Test Dataset')

BVSP_SVR_forecast_best_model<-BVSP_forecast_tune$best.model
BVSP_SVR_forecast_predicted_best_model<- predict(BVSP_SVR_forecast_best_model,BVSP_SVR_forecast_Data)
...
plot(abs(as.numeric(BVSP2_returns$`diff(log(BVSP2$Close))`))~BVSP2$Date,type='l',col='darkblue',
     main='IBOVESPA SVR Test', xlab='Date',
     ylab='Return',ylim=c(0,0.1))
lines(abs(BVSP_SVR_forecast)~
        BVSP2$Date,col='darkorange')
legend('topleft',
       legend=c('Absolute Actual Return','SVR Forecast'),
       col=c('darkblue','darkorange'),
       pch = c(9,9))
...
# BVSP Training Error function for Optimal model
BVSP_SVR_forecast<-as.data.frame(BVSP_SVR_forecast)
BVSP_actual_returns_test<-as.data.frame(BVSP_actual_returns_test)

for (i in 1:nrow(BVSP_actual_returns_test)) {
  d<-abs(BVSP_SVR_forecast[i,1]-BVSP_actual_returns_test[i,1])
  d<-as.data.frame(d)
...
for (i in 1:nrow(BVSP_actual_returns_test)) {
  d<-abs(BVSP_SVR_forecast[i,1]-BVSP_actual_returns_test[i,1])**2
  d<-as.data.frame(d)
