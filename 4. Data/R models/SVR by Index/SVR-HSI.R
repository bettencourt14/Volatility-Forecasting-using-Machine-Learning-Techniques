...
nrow(Brent)
nrow(HSI1)
colnames(HSI1)<-c('Date','Close','Return','Volume')
...
HSI_Brent  
str(HSI_Brent)
colnames(HSI_Brent)<- c('Date','Close HSI','Return HSI','Lag Vol','Close Brent','Return Brent')
head(HSI_Brent)
...
str(HSI_Brent_Corn)
colnames(HSI_Brent_Corn)<- c('Date','Close HSI','Return HSI','Lag Vol',
...
colnames(HSI_SVR_Data)<-c('HSI Return','Brent Return','Corn Return','Lag Vol')
head(HSI_SVR_Data)
...
plot(abs(as.numeric(HSI_Brent_Corn$`Return HSI`))~HSI_Brent_Corn$Date,type='l',col='grey',
     main='Actual Absolute Returns vs SVR model', xlab='Date',ylab='Return')
lines(abs(HSI_SVR_predicted_best_model)~
        HSI_Brent_Corn$Date,col='Orange')
...
HSI_SVR_forecast_Data<-cbind(HSI_forecast_retuns,HSI_forecast_Brent_Return,
...
colnames(HSI_SVR_forecast_Data)<-c('HSI Return','Brent Return', 'Corn Return','Lag Vol')
...
HSI_forecast_tune<-tune(svm,HSI_SVR_Data$`HSI Return`~
                           HSI_SVR_Data$`Brent Return`+
                           HSI_SVR_Data$`Corn Return`+
                           HSI_SVR_Data$`Lag Vol`,
...
plot(HSI_forecast_tune)

HSI_SVR_forecast_best_model<-HSI_forecast_tune$best.model
HSI_SVR_forecast_predicted_best_model<- predict(HSI_SVR_best_model,HSI_SVR_Data)
...
plot(abs(as.numeric(HSI2_returns$`diff(log(HSI2$Close))`))~HSI2$Date,type='l',col='grey',
     main='Actual Absolute Returns vs SVR model for Test', xlab='Date',
     ylab='Return',ylim=c(0,0.1))
lines(abs(HSI_SVR_forecast)~
        HSI2$Date,col='Orange')

# HSI Training Error function for Optimal model
HSI_SVR_forecast_predicted_best_model<-as.data.frame(HSI_SVR_forecast_predicted_best_model)
HSI_actual_returns_test<-as.data.frame(HSI_actual_returns_test)

for (i in 1:nrow(HSI_actual_returns_test)) {
  d<-abs(HSI_SVR_forecast_predicted_best_model[i,1]-HSI_actual_returns_test[i,1])
  d<-as.data.frame(d)
...
for (i in 1:nrow(HSI_actual_returns_test)) {
  d<-abs(HSI_SVR_forecast_predicted_best_model[i,1]-HSI_actual_returns_test[i,1])**2
  d<-as.data.frame(d)
