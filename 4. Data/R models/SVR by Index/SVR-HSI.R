...
min(Brent_returns)
nrow(Brent_returns)
plot(Brent_returns$Brent_returns,type='l')
...
plot(abs(as.numeric(HSI_Brent_Corn$`Return HSI`))~HSI_Brent_Corn$Date,type='l',col='grey',
     main='Hang Seng Actual Absolute Returns vs SVR model', xlab='Date',ylab='Return')
...
plot(abs(as.numeric(HSI2_returns$`diff(log(HSI2$Close))`))~HSI2$Date,type='l',col='grey',
     main='Hang Seng Actual Absolute Returns vs SVR model for Test', xlab='Date',
