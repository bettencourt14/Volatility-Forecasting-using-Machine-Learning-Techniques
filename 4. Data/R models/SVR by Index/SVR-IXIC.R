...
plot(IXIC_tune, main='NASDAQ SVR Model Performance on Training Dataset',col='darkblue')
...
plot(abs(as.numeric(IXIC_Brent_Corn$`Return IXIC`))~IXIC_Brent_Corn$Date,type='l',col='darkblue',
     main='NASDAQ SVR Training', xlab='Date',ylab='Volatility')
lines(abs(IXIC_SVR_predicted_best_model)~
        IXIC_Brent_Corn$Date,col='darkorange')
legend('topleft',
...
plot(IXIC_forecast_tune, main=' NASDAQ SVR Model Performance Test Dataset')
...
plot(abs(as.numeric(IXIC2_returns$`diff(log(IXIC2$Close))`))~IXIC2$Date,type='l',col='darkblue',
     main='NASDAQ SVR Test', xlab='Date',
     ylab='Return',ylim=c(0,0.1))
lines(abs(IXIC_SVR_forecast)~
        IXIC2$Date,col='darkorange')
legend('topleft',
       legend=c('Absolute Actual Return','SVR Forecast'),
       col=c('darkblue','darkorange'),
       pch = c(9,9))
...
# IXIC Training Error function for Optimal model
IXIC_SVR_forecast<-as.data.frame(IXIC_SVR_forecast)
IXIC_actual_returns_test<-as.data.frame(IXIC_actual_returns_test)

for (i in 1:nrow(IXIC_actual_returns_test)) {
  d<-abs(IXIC_SVR_forecast[i,1]-IXIC_actual_returns_test[i,1])
  d<-as.data.frame(d)
...
for (i in 1:nrow(IXIC_actual_returns_test)) {
  d<-abs(IXIC_SVR_forecast[i,1]-IXIC_actual_returns_test[i,1])**2
  d<-as.data.frame(d)
