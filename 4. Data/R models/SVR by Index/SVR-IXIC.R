...
nrow(Brent)
nrow(IXIC1)
colnames(IXIC1)<-c('Date','Close','Return','Volume')
...
IXIC_Brent  
str(IXIC_Brent)
colnames(IXIC_Brent)<- c('Date','Close IXIC','Return IXIC','Lag Vol','Close Brent','Return Brent')
head(IXIC_Brent)
...
str(IXIC_Brent_Corn)
colnames(IXIC_Brent_Corn)<- c('Date','Close IXIC','Return IXIC','Lag Vol',
...
colnames(IXIC_SVR_Data)<-c('IXIC Return','Brent Return','Corn Return','Lag Vol')
head(IXIC_SVR_Data)
...
lines(abs(IXIC_SVR_predicted_best_model$IXIC_SVR_predicted_best_model)~
...
IXIC_SVR_forecast_Data<-cbind(IXIC_forecast_retuns,IXIC_forecast_Brent_Return,
...
colnames(IXIC_SVR_forecast_Data)<-c('IXIC Return','Brent Return', 'Corn Return','Lag Vol')
...
IXIC_forecast_tune<-tune(svm,IXIC_SVR_Data$`IXIC Return`~
                           IXIC_SVR_Data$`Brent Return`+
                           IXIC_SVR_Data$`Corn Return`+
                           IXIC_SVR_Data$`Lag Vol`,
...
IXIC_SVR_forecast_predicted_best_model<- predict(IXIC_SVR_best_model,IXIC_SVR_Data)
