...
nrow(Brent)
nrow(NSEI1)
colnames(NSEI1)<-c('Date','Close','Return','Volume')
...
NSEI_Brent  
str(NSEI_Brent)
colnames(NSEI_Brent)<- c('Date','Close NSEI','Return NSEI','Lag Vol','Close Brent','Return Brent')
head(NSEI_Brent)
...
str(NSEI_Brent_Corn)
colnames(NSEI_Brent_Corn)<- c('Date','Close NSEI','Return NSEI','Lag Vol',
...
colnames(NSEI_SVR_Data)<-c('NSEI Return','Brent Return','Corn Return','Lag Vol')
head(NSEI_SVR_Data)
...
NSEI_SVR_forecast_Data<-cbind(NSEI_forecast_retuns,NSEI_forecast_Brent_Return,
...
colnames(NSEI_SVR_forecast_Data)<-c('NSEI Return','Brent Return', 'Corn Return','Lag Vol')
...
NSEI_forecast_tune<-tune(svm,NSEI_SVR_Data$`NSEI Return`~
                           NSEI_SVR_Data$`Brent Return`+
                           NSEI_SVR_Data$`Corn Return`+
                           NSEI_SVR_Data$`Lag Vol`,
...
NSEI_SVR_forecast_predicted_best_model<- predict(NSEI_SVR_best_model,NSEI_SVR_Data)
